...
apiRouter.post('/products', authenticate, (req: AuthenticatedRequest, res: Response) => {
  ...
  const finalIndividualCode = (storeIdCode || individualCode || String(Math.floor(10000 + Math.random() * 90000))).trim();
  ...
  for (let i = 0; i < countToCreate; i++) {
    ...
    const newProduct: Product = {
      id: prodId,
      productType: productType || (unit === 'KIT' ? 'KIT' : 'UNIDADE'),
      sku: finalBoxSku,
      boxSku: finalBoxSku,
      ...
      unit: unit || (productType === 'KIT' ? 'KIT' : 'UN'),
      ...
      kitItems: Array.isArray(kitItems) ? kitItems : undefined,
      kitItemCount: Number(kitItemCount) || (Array.isArray(kitItems) ? kitItems.length : undefined),
      createdAt: new Date().toISOString().split('T')[0],
...
apiRouter.put('/products/:id', authenticate, (req: AuthenticatedRequest, res: Response) => {
  ...
  if (kitItems !== undefined) product.kitItems = kitItems;
  if (kitItemCount !== undefined) product.kitItemCount = Number(kitItemCount);
...
apiRouter.post('/purchases', authenticate, requireAdmin, (req: AuthenticatedRequest, res: Response) => {
  ...
  for (const item of items) {
    ...
    let baseStoreId = (product.storeIdCode || product.individualCode || String(Math.floor(10000 + Math.random() * 90000))).trim();
